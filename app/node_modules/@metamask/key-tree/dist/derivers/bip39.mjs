...
import { PUBLIC_KEY_GUARD } from "../guard.mjs";
 ...
 */
export function multipathToBip39Mnemonic(value) {
    if (value instanceof Uint8Array) {
        return value;
    }
    assert(value.startsWith('bip39:'), 'Invalid HD path segment: The BIP-39 path must start with "bip39:".');
 ...
 */
export async function getDerivationPathWithSeed({ path, curve: curveName }, cryptographicFunctions) {
...
export async function deriveChildKey({ path, curve, network }, cryptographicFunctions) {
    assert(path instanceof Uint8Array, 'Invalid path: The path must be a Uint8Array.');
    switch (curve.masterNodeGenerationSpec) {
        case 'slip10':
...
export async function createBip39KeyFromSeed(seed, curve, network, cryptographicFunctions) {
    ...
    const masterFingerprint = getFingerprint(curve.compressPublicKey(publicKey), curve.compressedPublicKeyLength);
